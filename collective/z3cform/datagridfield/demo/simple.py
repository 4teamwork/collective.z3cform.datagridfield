...
class IPerson(Interface):
    ...
    address = schema.List(title=u'Addresses',
                          value_type=DictRow(
                              title=u'Address', schema=IAddress),
                          required=True)
...
class EditForm(form.EditForm):
    ...
    def dumpOutput(self, data):
        ...
        logger.debug('Dumping out extracted addresses')
        for entry in address:
            logger.debug(entry)

        logger.debug('Dumping out raw HTTP POST form data')
        for k, v in self.request.form.items():
            logger.debug('{0:s}: {1:s}'.format(k, v))
